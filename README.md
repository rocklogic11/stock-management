...
| 商品管理 | 商品CRUD、SKU自动生成(KW-TOY-XXX)、二维码生成与打印、扫码获取商品编号(barcode)、商品图片上传(最多4张) |
| 入库管理 | 入库单创建、扫码添加商品(支持SKU/商品编号)、确认入库、加权平均成本计算 |
...
| 角色权限 | 3 | 角色CRUD |
| 商品分类 | 4 | 分类CRUD |
| 商品管理 | 7 | 商品CRUD/二维码生成/扫码获取商品编号/图片上传 |
| 入库管理 | 6 | 入库单CRUD/确认入库 |
...
| 版本 | 日期 | 内容 |
|------|------|------|
| v1.2 | 2026-05-31 | 新增商品编号(barcode)扫码填充、商品图片上传(最多4张)、入库支持商品编号匹配、OSS上传模块 |
| v1.1 | 2026-05-31 | 项目整理：文档统一命名、清理临时文件、修正过时配置、Git初始化、推送到GitHub |
